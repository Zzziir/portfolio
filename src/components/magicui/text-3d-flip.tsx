...
/**
 * Magic UI's Text3DFlip: each character sits on a little 3D cube and flips on
 * hover, staggered from a chosen origin, revealing the same glyph on the next
 * face. From the registry, with a reduced-motion guard added. Faces need an
 * opaque background that matches their surface (pass it via
 * text/flipTextClassName) so the cube edges stay hidden mid-flip.
 */
...
const Text3DFlip = ({
  ...
  const handleHoverStart = useCallback(async () => {
    ...
    // respect reduced motion: no flip
    if (
  ...
  return (
    ...
    >
      ...
      {characters.map((wordObj, wordIndex) => (
        // perspective (scaled to the font via lh) gives the per-letter flip real
        // depth instead of a flat, near-invisible orthographic rotation
        <span key={wordIndex} className="inline-flex [perspective:8lh]">
