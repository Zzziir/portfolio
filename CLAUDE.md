...
**No em-dash asides (binding).** Never use an em-dash (Unicode U+2014) as a
sentence-level aside or pause anywhere in this repo (copy, metadata/titles, UI
text, comments, docs). That usage reads as AI-generated. Use a comma, colon,
parentheses, or a spaced hyphen (` - `) instead.

**Allowed (expected, not AI-looking):** a plain hyphen (`-`) in compound
modifiers (`one-year-old`, `two-storey`, `full-stack`, `cross-platform`) and in
numeric ranges (`2,000-3,000`, `2024-2026`); an en-dash (U+2013) is also fine for
a range if preferred. The `→` arrow (e.g. `0 → 12k`) is fine; it is not a dash.
